    ...
    <form x-data="{
        ...
        <div class="relative flex-1 w-full" x-data="{ isOpen: $data.isOpen, search: $data.search }" @click.away="isOpen = false">
            ...
            <input type="text"
                ...
                placeholder="{{ trans('Search location...') }}" x-model="search" @focus="isOpen = true"
                wire:model.debounce.300ms="searchTerm" @keydown.escape.window="isOpen = false">
            <!-- Dropdown -->
            <div x-show="isOpen"
        ...
        <!-- Search Button -->
        <button type="submit"
            class="w-full md:w-auto px-6 py-3 bg-[#FCFE04] text-gray-800 rounded-lg hover:bg-[#e6e600] transition-colors flex items-center justify-center gap-2">
