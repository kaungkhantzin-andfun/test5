...
<div {{$attributes}} class="flex items-center justify-center gap-1 mt-2 cursor-pointer lg:hidden label info">
	@if (count($properties) > 0)

	<x-loading-indicator />

	<x-icon.solid-icon wire:loading.remove
	...
	{{__('Close this to see updated result.')}}
	@else

	<x-loading-indicator />

	<x-icon.solid-icon wire:loading.remove class="text-red-600" :add-class="true"
