...
<body class="antialiased">
    <div x-cloak x-data="{ sidebarOpen: $persist(true), mobileShow: false }" class="lg:flex">
            ...
            <nav class="mt-3 space-y-1 max-w-[250px]">
                ...
                <x-dashboard.nav-item route="dashboard/enquiries"
                    ...
                    @if (Auth::user()->role === 'remwdstate20')
                        ...
                        @if ($enquiryCount > 0)
                            <span
                                class="flex justify-center w-5 h-5 text-sm font-bold text-white bg-red-600 border border-white rounded-full">
                                {{ $enquiryCount }}
                            </span>
                        @endif
                    @else
                        @php
                            $enquiryCount = App\Models\Enquiry::whereNull('status')
                ...
                @if (Auth::user()->role == 'remwdstate20')
                    <div class="nav_sub">
                        ...
                        <x-dashboard.nav-item route="dashboard/types/property"
                            create-route="dashboard/types/property/create"
        ...
        <div
            class="flex flex-col flex-1 min-h-screen {{ Request::is('*/create') || Request::is('*/edit') ? 'overflow-y-auto' : '' }}">
            <header
                class="flex flex-col gap-3 px-4 py-2 bg-white shadow md:justify-between md:items-center md:flex-row">
                <div class="flex items-center gap-4">
                    ...
                    <div @click="sidebarOpen = true; mobileShow = !mobileShow"
                        class="flex gap-2 cursor-pointer lg:hidden">
                        <button class="text-blue-500 focus:outline-none">
                            <x-icon.icon :path="'M4 6h16M4 12h16M4 18h7'" />
                        </button>
                    ...
                    @if (!empty($addNew))
                        <div class="relative flex">
                            <a href="{{ LaravelLocalization::localizeUrl($addNew) }}"
                                class="flex justify-between gap-1 bg-gradient-to-r blue-gradient btn">
                                {{ __('Add New') }}
                ...
                @if (Request::is('*dashboard/properties'))
                    <div class="flex items-center gap-2 text-sm md:flex-col xl:flex-row sm:text-base">
                        @if (!empty(Auth::user()->credit))
                            <p>{!! __(
                                'You have <strong>' .
                                    number_format(Auth::user()->credit) .
                                    ' credit points</strong>
                                                                                which you can use to feature your properties.',
                            ) !!}</p>
                        @else
                            <p>{{ __("You don't have any point left!") }}</p>
                        @endif
                        <a class="font-semibold text-blue-600 hover:underline"
                            href="{{ LaravelLocalization::localizeUrl(route('dashboard.top-up')) }}">{{ __('Top Up
                                                                                Now!') }}</a>
                    </div>
                @endif
                    ...
                    <ul class="flex">
                        @if (LaravelLocalization::getCurrentLocale() == 'my')
                            <a rel="alternate" hreflang="en"
                                ...
                                <img class="w-8 rounded shadow" src="{{ asset('assets/images/us.svg') }}"
                                    alt="US Country Flag">
                            </a>
                        @else
                            <a rel="alternate" hreflang="my"
                                href="{{ LaravelLocalization::getLocalizedURL('my', null, [], true) }}">
                                <img class="w-8 rounded shadow" src="{{ asset('assets/images/myanmar.svg') }}"
                                    alt="Myanmar Country Flag">
                            </a>
                        @endif
                    </ul>

                    <a class="text-gray-700" href="{{ LaravelLocalization::localizeUrl(route('home')) }}">
                        <x-icon.icon :path="'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'" class="w-5 h-5" />
                    </a>
